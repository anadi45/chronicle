...
function App() {
  ...
  const setCaptureEnabled = async (enabled: boolean) => { try { await invoke(enabled ? "start_capture" : "stop_capture"); } catch (error) { console.error(error); return; } setCaptureState(enabled); };
