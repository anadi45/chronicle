...
pub fn capture_window_png(window_handle: isize) -> Result<Vec<u8>, String> {
    ...
    unsafe { GetClientRect(hwnd, &mut rect).map_err(|error| error.to_string())?; }
    let width = (rect.right - rect.left) as i32;
    let height = (rect.bottom - rect.top) as i32;
    if width <= 0 || height <= 0 || (width as i64 * height as i64) > 16_000_000 { return Err("window dimensions are invalid or exceed screenshot limit".into()); }
    let source = unsafe { GetDC(Some(hwnd)) };
    if source.is_invalid() { return Err("GetDC failed".into()); }
...
pub(crate) fn encode_png_rgba(width: u32, height: u32, rgba: &[u8]) -> Result<Vec<u8>, String> {
    ...
    for (index, chunk) in scanlines.chunks(65_535).enumerate() { let final_block = index == (scanlines.len() - 1) / 65_535; compressed.push(if final_block { 1 } else { 0 }); let len = chunk.len() as u16; compressed.extend_from_slice(&len.to_le_bytes()); compressed.extend_from_slice(&(!len).to_le_bytes()); compressed.extend_from_slice(chunk); }
    compressed.extend_from_slice(&adler32(&scanlines).to_be_bytes());
    let mut png = vec![137,80,78,71,13,10,26,10];
    let mut header = Vec::with_capacity(13); header.extend_from_slice(&width.to_be_bytes()); header.extend_from_slice(&height.to_be_bytes()); header.extend_from_slice(&[8, 6, 0, 0, 0]); append_chunk(&mut png, b"IHDR", &header); append_chunk(&mut png, b"IDAT", &compressed); append_chunk(&mut png, b"IEND", &[]); Ok(png)
}

fn append_chunk(output: &mut Vec<u8>, kind: &[u8; 4], data: &[u8]) { output.extend_from_slice(&(data.len() as u32).to_be_bytes()); output.extend_from_slice(kind); output.extend_from_slice(data); let mut crc_input = Vec::with_capacity(4 + data.len()); crc_input.extend_from_slice(kind); crc_input.extend_from_slice(data); output.extend_from_slice(&crc32(&crc_input).to_be_bytes()); }
