...
pub fn initialize(window_handle: isize) -> Result<D3d11CaptureSession, String> {
    ...
    unsafe { D3D11CreateDevice(None, D3D_DRIVER_TYPE_HARDWARE, HMODULE::default(), D3D11_CREATE_DEVICE_FLAG(0), Some(&[D3D_FEATURE_LEVEL_11_0]), 7, Some(&mut device), None, Some(&mut context)).map_err(|error| format!("unable to create D3D11 device: {error}"))?; }
    let device = device.ok_or("D3D11 returned no device")?;
    let context = context.ok_or("D3D11 returned no immediate context")?;
    let dxgi_device: IDXGIDevice = device.cast().map_err(|error| format!("unable to cast DXGI device: {error}"))?;
    ...
    Ok(D3d11CaptureSession { frame_pool, session, device, context })
}
...
impl D3d11CaptureSession {
    pub fn capture_next_frame_png(&self) -> Result<Vec<u8>, String> {
        ...
        use windows::core::Interface;
        use windows::Win32::Graphics::Direct3D11::{D3D11_CPU_ACCESS_READ, D3D11_MAP_READ, D3D11_TEXTURE2D_DESC, D3D11_USAGE_STAGING, ID3D11Texture2D};
        use windows::Win32::System::WinRT::Direct3D11::IDirect3DDxgiInterfaceAccess;
        let frame = self.frame_pool.TryGetNextFrame().map_err(|error| format!("unable to acquire capture frame: {error}"))?;
        let surface: IDirect3DSurface = frame.Surface().map_err(|error| format!("unable to access capture surface: {error}"))?;
        let access: IDirect3DDxgiInterfaceAccess = surface.cast().map_err(|error| format!("unable to access DXGI surface: {error}"))?;
        let texture: ID3D11Texture2D = unsafe { access.GetInterface().map_err(|error| format!("unable to access D3D11 texture: {error}"))? };
        let mut desc = D3D11_TEXTURE2D_DESC::default(); unsafe { texture.GetDesc(&mut desc); }
        desc.Usage = D3D11_USAGE_STAGING;
        desc.BindFlags = 0;
        desc.CPUAccessFlags = D3D11_CPU_ACCESS_READ.0 as u32;
        desc.MiscFlags = 0;
        let mut staging = None;
        unsafe { self.device.CreateTexture2D(&desc, None, Some(&mut staging)).map_err(|error| format!("unable to create CPU staging texture: {error}"))?; }
        ...
        unsafe { self.context.Map(&staging, 0, D3D11_MAP_READ, 0, Some(&mut mapped)).map_err(|error| format!("unable to map captured frame: {error}"))?; }
        let mut pixels = vec![0u8; (desc.Width as usize) * (desc.Height as usize) * 4];
        unsafe { for row in 0..desc.Height as usize { let source = core::slice::from_raw_parts((mapped.pData as *const u8).add(row * mapped.RowPitch as usize), desc.Width as usize * 4); pixels[row * desc.Width as usize * 4..(row + 1) * desc.Width as usize * 4].copy_from_slice(source); } self.context.Unmap(&staging, 0); }
